...
const Current = () => {
    ...
    const fetchrecentvalues = async () => {
      try {
          ...
          if (!response.error) {
            ...
            // Update only the current value in the state
            // Using 'y' field as current since current field is not present in JSON
            setcurrent(firstSensorValue.y ? firstSensorValue.y : 0.0);            
  ...
  useEffect(() => {
//    console.log(current); // You can remove or modify this line
    setValue(current);
    setIsPopped(true);
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="subtitle1"><h4>Yellow Phase</h4></Typography>
      </div>

      <StyledIcon>
        <Iconify icon={"Zap"} width={24} height={24} />
      </StyledIcon>
