...
const Voltage = () => {
    ...
    const fetchrecentvalues = async () => {
      try {
          ...
          if (!response.error) {
            ...
            // Update only the voltage value in the state
            // Using 'r' field as voltage since voltage field is not present in JSON
            setvoltage(firstSensorValue.r ? firstSensorValue.r : 0.0);            
  ...
  useEffect(() => {
//    console.log(voltage); // You can remove or modify this line
    setValue(voltage);
    setIsPopped(true);
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="subtitle1"><h4>Voltage</h4></Typography>
      </div>

      <StyledIcon>
        <Iconify icon={"Lightning"} width={24} height={24} />
      </StyledIcon>
