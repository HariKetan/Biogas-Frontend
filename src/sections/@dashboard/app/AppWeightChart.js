...
export default function AppWeightChart() {
  ...
  useEffect(() => {
    const fetchWeightData = async () => {
      try {
        ...
        if (!response.error && response.data && response.data.length > 0) {
          ...
          const currentWeightValue = firstSensorValue.weight ? parseFloat(firstSensorValue.weight) : 0;
          ...
          if (response.data.length === 1) {
            ...
            const baseWeight = currentWeightValue;
            const now = new Date();
            ...
            chartDataPoints = reversedData.map((item, index) => {
              ...
              console.log(`Data point ${index}: weight = ${weight}`);
              return weight;
            });
          ...
          console.log('Final Weight Data Array:', chartDataPoints);
    ...
    const interval = setInterval(fetchWeightData, 30000); // Update every 30 seconds
  ...
  const chartOptions = useChart({
    ...
    yaxis: {
      ...
      min: chartData[0]?.data?.length > 0 ? (Math.min(...chartData[0].data) - 1) : 0,
      max: chartData[0]?.data?.length > 0 ? (Math.max(...chartData[0].data) + 1) : 100,
      labels: {
        style: {
          fontSize: '11px',
          color: '#263238'
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="h6">Current: {currentWeight.toFixed(2)} KG</Typography>
      </div>
    </Card>
  );
} 